    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        <Select value={filterType} onValueChange={setFilterType}>
          <SelectTrigger className="w-40 bg-card border-border text-sm"><SelectValue placeholder="Signal type" /></SelectTrigger>
        ...
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          <SelectTrigger className="w-36 bg-card border-border text-sm"><SelectValue placeholder="Status" /></SelectTrigger>
          <SelectContent className="bg-card border-border">
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className={cn("text-xs font-bold px-2.5 py-1 rounded-lg border font-mono min-w-20 text-center", signalColors[signal.signal_type])}>
                    {signal.signal_type}
                  </span>
                  <div>
                    <div className="font-bold text-sm">{signal.asset_symbol} <span className="text-muted-foreground font-normal">{signal.asset_name}</span></div>
                    <div className="text-xs text-muted-foreground mt-0.5 max-w-sm truncate">{signal.summary}</div>
                  </div>
                </div>
              ...
              <div className="border-t border-border p-4 space-y-4 bg-secondary/20">
                ...
                {signal.reward_risk_ratio && (
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="font-mono font-bold text-primary">{signal.reward_risk_ratio.toFixed(2)}:1</span>
                    {signal.suggested_position_size_pct && (
                      <>
                        <span className="text-muted-foreground ml-2">Position Size:</span>
                        <span className="font-mono font-bold text-foreground">{signal.suggested_position_size_pct.toFixed(1)}% of portfolio</span>
                      </>
                    )}
                    {signal.timeframe && (
                      <>
                        <span className="text-muted-foreground ml-2">Timeframe:</span>
                        <span className="font-mono font-bold">{signal.timeframe}</span>
                      </>
                    )}
                  </div>
                )}
                ...
                {signal.status === 'active' && (
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => handleStatusChange(signal, 'executed')} className="bg-green-500/20 text-green-400 border border-green-500/30 hover:bg-green-500/30">Accept & Execute</Button>
                    <Button size="sm" variant="outline" onClick={() => handleStatusChange(signal, 'rejected')}>Reject</Button>
                    <Button size="sm" variant="outline" onClick={() => handleStatusChange(signal, 'expired')}>Expire</Button>
                  </div>
                )}
              </div>
            )}
